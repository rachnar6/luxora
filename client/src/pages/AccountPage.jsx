...
const AccountPage = () => {
    ...
    // Helper to determine initial tab
    const getInitialTab = useCallback(() => {
        if (!user) return 'profile';
        if (user.isSeller) return 'seller-dashboard';
        if (!user.isAdmin) return 'seller-apply';
        return 'profile';
     }, [user]);
    ...
    const [sellerStatus, setSellerStatus] = useState('Not Applied'); // Default correctly
    ...
    // Effects
    useEffect(() => {
        // console.log("AccountPage User Effect - User:", user, "Auth Loading:", authLoading);
        if (!authLoading) {
            if (user) {
                setName(user.name || '');
                setEmail(user.email || '');
                ...
                // --- MODIFIED SELLER STATUS LOGIC ---
                let currentStatus = 'Not Applied'; // Default
                const appStatus = user.sellerApplicationStatus;

                // Only use appStatus if it's one of the expected valid statuses
                if (appStatus && ['Pending', 'Approved', 'Rejected'].includes(appStatus)) {
                    currentStatus = appStatus;
                } else if (user.isSeller) {
                    // If no valid appStatus, but user IS a seller, treat as Approved
                    currentStatus = 'Approved';
                }
                // Otherwise, it remains 'Not Applied' (handles null, undefined, 'none', etc.)
                setSellerStatus(currentStatus);
                // --- END MODIFIED LOGIC ---

                const correctInitialTab = getInitialTab();
                if (activeTab !== correctInitialTab && !editMode) {
                    // console.log(`Updating activeTab from ${activeTab} to ${correctInitialTab}`);
                    setActiveTab(correctInitialTab);
                }
            } else {
    ...
    }, [user, authLoading, navigate, getInitialTab, activeTab, editMode]);
    ...
    const handleApplySeller = async () => {
        ...
        try {
            ...
            setSellerStatus(data.sellerApplicationStatus || 'Pending');
    ...
    // --- Add console logs for debugging state just before render ---
    // console.log('--- AccountPage Render State ---');
    // console.log('Current activeTab:', activeTab);
    // console.log('Current sellerStatus:', sellerStatus);
    // console.log('Is user loaded?', !!user);
    // console.log('Is user admin?', user?.isAdmin);
    // console.log('Is user seller?', user?.isSeller);
                 ...
                 <div>
                     ...
                     {activeTab === 'seller-apply' && !user?.isSeller && !user?.isAdmin && (
                          <div className="animate-fade-in-down">
                              ...
                              {sellerStatus === 'Not Applied' && ( // Check for 'Not Applied' specifically
                                  <div>
